		...
		protected:
			...
			void Insert(T& object, const Vector3& objectPos, const Vector3& objectSize, int depthLeft, int maxSize) {
				...
				if (!CollisionDetection::AABBTest(objectPos, Vector3(position.x, 0, position.y), objectSize, Vector3(size.x, 1000.0f, size.y))) {
					return;	//if an object doesn’t overlap a node, it also cannot overlap any of the children of that node
				}
				...
				if (children) { // not a leaf node , just descend the tree
					...
					if ((int)contents.size() > maxSize && depthLeft > 0) {	// unless we’ve got to the maximum tree depth we are allowing for the quadtree
						if (!children) {
							//a quad tree node has a maximum number	of objects it can contain, and once that has been exceeded, the node must split itself up
							Split();
			...
			void Split() {	// split up into 4 child nodes that when combined, fill the same area
			...
			void OperateOnContents(QuadTreeFunc& func) {
				if (children) {	// check to see if any particular quad tree node has children
					for (int i = 0; i < 4; ++i) {
						children[i].OperateOnContents(func);	// recursively call the method until we drop down to some leaf nodes
					}
				}
				else {
					if (!contents.empty()) {	// to save calling any passed in functions on empty lists
						func(contents);
					}
				}
			}
